...
# function to decide whether or not to proceed with fitting the planet's astrometry
# the coefficients were determined empirically, tuned so that the SNR distribution is ~log-uniform
def get_acceptance_bool(astrom_SNR, coeffs=np.array([-2.83204204,  0.85311645])):
    acceptance_prob = 1.5453696188751223e-05/(10**np.polyval(coeffs, np.log10(astrom_SNR)))
    if np.random.rand() < acceptance_prob/astrom_SNR:
        return True
    return False
...
if __name__ == "__main__":
    ...
    for _ in range(1_000_000):
        ...
        poisson_num_planets = np.random.poisson(occ_rates) # Poisson-sample number of planets
        for i in range(len(poisson_num_planets)):
            if poisson_num_planets[i] > 0: # stars around which to assign planets
                planet_mass, semi_a = -1, -1

                # rejection sampling to choose planet masses semi-major axes
                while planet_mass < 0 and semi_a < 0:
                    ...
                    if np.random.rand() < planet_occurrence(masses[i], rand_a)/(max_occ_rates[i]):
                        ...
                        period = 365.25*(semi_a**1.5)/((masses[i] + planet_mass)**0.5) # days
        ...
        for i in range(num_stars):
            if planets[i][0] != 0.0:
                ...
                sigma_fov = piecewise(np.array([Gs[i]]), 53.89888779, 14.0)[0]

                # proceed with probability from 'get_acceptance_bool' function
                if 0.1 < astrom_signal/sigma_fov < 100.0 and planets[i][1]/365.25 < 4.0 and get_acceptance_bool(astrom_signal/sigma_fov):
                    # generate synthetic astrometric data
                    t_ast_yr, psi, plx_factor, ast_obs, ast_err = gaiamock.predict_astrometry_luminous_binary(ra=ras[i], dec=decs[i], parallax=parallaxes[i], pmra=pmras[i], pmdec=pmdecs[i], m1=masses[i], m2=planets[i][0], period=planets[i][1], Tp=planets[i][2], ecc=planets[i][3], omega=planets[i][4], inc=planets[i][5], w=planets[i][6], phot_g_mean_mag=Gs[i], f=0.0, data_release='dr4', c_funcs=c_funcs)
                    ...
                    with open("data/chi2_vs_SNR_DR4/file" + str(run_num) + "_" + str(recorded_count) + ".pkl", "wb") as f:
