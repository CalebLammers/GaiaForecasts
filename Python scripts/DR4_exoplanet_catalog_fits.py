...
if __name__ == "__main__":
    ...
    out_path = "data/Gaia_real_stars_recover_DR4/file" + str(run_num) + ".pkl"
    ...
    with open(out_path, "ab") as fout: # open file, which will be written to continually
        for i in range(len(source_ids)):
            if planets[i][0] != 0.0:
                ...
                # proceed if N_sigma > 0.5 and Porb < 7 years
                if astrom_signal/sigma_fov > 0.5 and planets[i][1]/365.25 < 7.0:
                    # generate synthetic astrometric data
                    t_ast_yr, psi, plx_factor, ast_obs, ast_err = gaiamock.predict_astrometry_luminous_binary(ra=ras[i], dec=decs[i], parallax=parallaxes[i], pmra=pmras[i], pmdec=pmdecs[i], m1=masses[i], m2=planets[i][0], period=planets[i][1], Tp=planets[i][2], ecc=planets[i][3], omega=planets[i][4], inc=planets[i][5], w=planets[i][6], phot_g_mean_mag=G_mags[i], f=0.0, data_release='dr4', c_funcs=c_funcs)
